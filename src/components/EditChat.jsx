...
const EditChat = forwardRef(({x, y, changeChatTitle, deleteChat}, ref) => {
  ...
  return (
    <div ref={ref} className="fixed shadow-md rounded-xl bg-gray-200 p-2 gap-2" style={{ top: y, left: x }}>
      <button onClick={changeChatTitle} className="flex group items-center hover:bg-gray-400/30 w-full px-2 py-1 rounded-lg text-sm">
        <FiEdit3 size={15} className="mr-2"/>
        Rename
      </button>
      <hr className="mx-auto m-1 border-t border-gray-400 max-w-[70%]"></hr>
        <button onClick={deleteChat} className="flex group items-center hover:bg-red-500/10 w-full px-2 py-1 rounded-lg text-sm text-red-700">
